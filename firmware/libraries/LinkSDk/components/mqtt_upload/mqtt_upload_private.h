...
typedef struct {
    ...
    uint32_t    uploaded_size;                      /* 已经上传的文件大小 */
...
#define MQTT_UPLOAD_OPEN_STREAM_PARAM_FMT        "{\"fileName\":\"%s\",\"fileSize\":%s,\"conflictStrategy\":\"%s\"}"
#define MQTT_UPLOAD_OPEN_STREAM_PARAM_FMT_MD5    "{\"fileName\":\"%s\",\"fileSize\":%s,\"conflictStrategy\":\"%s\",\"ficMode\":\"%s\",\"ficValue\":\"%s\"}"
#define MQTT_UPLOAD_SEND_DATA_PARAM_FMT          "{\"uploadId\":\"%s\",\"offset\":%s,\"bSize\":%s}"
...
#define MQTT_UPLOAD_DEFAULT_CRC64_LEN             (32)            /* MD5校验的长度*/

#define MQTT_UPLOAD_HEADER_LENGTH_BYTES           (2)             /* jsonstring的长度 */
#define MQTT_UPLOAD_PACKAGE_CHECK_BYTES           (2)             /* 数据CRC部分校验长度 */
