...
static int32_t _mqtt_upload_file_task_init(aiot_sysdep_portfile_t *sysdep, upload_file_task_handle_t *up_task, const char *file_name, 
        const uint32_t file_size, const char *mode, const char *digest, aiot_mqtt_upload_read_handler_t read_data_handler, void *userdata) {
...
static int32_t _mqtt_upload_send_request_init(mqtt_upload_handle_t *up_handle, upload_file_task_handle_t *up_task, char *file_name, 
    uint32_t file_size, const char *mode, const char *digest) {
    ...
    core_uint2str(file_size, size_string, NULL);
    src[0] = file_name;
    src[1] = size_string;
    src[2] = (char *)mode;
...
static int32_t _mqtt_upload_send_block_file(mqtt_upload_handle_t *up_handle, upload_file_task_handle_t *up_task, uint8_t *data, 
    const uint32_t block_size, const uint32_t offset) 
{
    ...
    uint32_t payload_len = block_size + strlen(head_payload) + sizeof(uint16_t) + sizeof(uint16_t);
        ...
        case AIOT_MQTT_UPLOADRECV_UPLOAD_REPLY :
            ...
            if (packet->desc.code == UPLOAD_FILE_OK) {
                ...
                if (data_len != 0 && data != NULL && 
                        (_mqtt_upload_block_size_is_valid(data_len, ((up_task->uploaded_size + data_len) >= up_task->file_size) ? 1 : 0))) {

                    if (_mqtt_upload_send_block_file(up_handle, up_task, data, data_len, up_task->uploaded_size) == STATE_SUCCESS) {
                        up_task->file_offset = up_task->uploaded_size;
            ...
            } else if (packet->desc.code == UPLOAD_FILE_FAILED_NOT_FILE_TASK) {
                /* 上传的文件任务不存在，重新请求初始化 */
                up_task->status = STATE_MQTT_UPLOAD_REQUEST_INIT;
                _mqtt_upload_resend_pub(up_handle, up_task);
            } else if (packet->desc.code == UPLOAD_FILE_FILE_BLOCK_DUPLICATION) { 
                /* 文件分片上传重复，云端沟通后暂做忽略处理 */
                core_log(sysdep, UPLOAD_FILE_FILE_BLOCK_DUPLICATION, "UPLOAD_FILE_FILE_BLOCK_DUPLICATION ignore\r\n");
            } else if (packet->desc.code == UPLOAD_FILE_FAILED_OFFSET_UNEQUAL) {
                /* 分片上传时offset不一致,可能超时重发照成包重复，忽略等待正确的应答包 */
                core_log(sysdep, UPLOAD_FILE_FAILED_OFFSET_UNEQUAL, "UPLOAD_FILE_FAILED_OFFSET_UNEQUAL ignore, wait ack ok.\r\n");
            } else
            {   
                if (packet->desc.code == UPLOAD_FILE_FAILED_SERVER_OTHER) {
                    /* 500异常的时候尝试两次重发，继续返回500失败结束上传 */
                    if (up_task->failed_count < 2) {
                        _mqtt_upload_resend_pub(up_handle, up_task);
                        break;
                    } else {
                        up_task->status = STATE_MQTT_UPLOAD_FAILED;   
                    }
                } else if (packet->desc.code == UPLOAD_FILE_FAILED_WHOLE_CHECK) {
                    up_task->status = STATE_MQTT_UPLOAD_FAILED_WHOLE_CHECK;
                } else if(packet->desc.code == UPLOAD_FILE_HAVE_FINISHED) {
                    up_task->status = STATE_MQTT_UPLOAD_FINISHED;
                } else if (up_task->status == UPLOAD_FILE_MESSAGE_LIMIT) {
                    up_task->status = STATE_MQTT_UPLOAD_FAILED_MESSAGE_LIGMIT;
                } else {
                    up_task->status = STATE_MQTT_UPLOAD_FAILED;
                }
                /* 其他异常发送失败，结束发送 */
...
static int32_t _mqtt_upload_resend_pub(mqtt_upload_handle_t *up_handle, upload_file_task_handle_t *up_task) {
    ...
    if (up_task->status == STATE_MQTT_UPLOAD_REQUEST_INIT) {
        res = _mqtt_upload_send_request_init(up_handle, up_task, up_task->file_name, up_task->file_size, up_task->mode, up_task->crc64);
    } else if (up_task->status == STATE_MQTT_UPLOAD_IS_UPLOADING && up_task->block_size != 0) {
        res = _mqtt_upload_send_block_file(up_handle, up_task, up_task->data, up_task->block_size, up_task->file_offset);
...
static int32_t _mqtt_upload_filelist_insert(mqtt_upload_handle_t *up_handle, upload_file_task_handle_t *up_task)
{
    ...
    node = sysdep->core_sysdep_malloc(sizeof(upload_file_task_handle_t), MQTT_UPLOAD_MODULE_NAME);
    if (node == NULL) {
        return STATE_SYS_DEPEND_MALLOC_FAILED;
    }
    memset(node, 0, sizeof(upload_file_task_handle_t));
    node->file_size = up_task->file_size;
    node->file_offset = up_task->file_offset;
    node->block_size = up_task->block_size;
    node->failed_count = up_task->failed_count;
    node->send_last_time = up_task->send_last_time;
    node->status = up_task->status;
    node->err_code = up_task->err_code;
    node->read_data_handler = up_task->read_data_handler;
    node->userdata = up_task->userdata;
    node->is_destory = up_task->is_destory;

    _mqtt_upload_creat_string_malloc(sysdep, up_task->file_name, strlen(up_task->file_name), (char **)&(node->file_name));
    _mqtt_upload_creat_string_malloc(sysdep, up_task->mode, MQTT_UPLOAD_DEFAULT_MODE_MAX_SIZE, (char **)&(node->mode));
    _mqtt_upload_creat_string_malloc(sysdep, up_task->crc64, MQTT_UPLOAD_DEFAULT_CRC64_LEN, (char **)&(node->crc64));
...
int32_t aiot_mqtt_upload_open_stream(void *handle, char *file_name, uint32_t file_size,
    const char *mode, void *digest, aiot_mqtt_upload_read_handler_t read_data_handler, void *userdata) {
    mqtt_upload_handle_t *up_handle = (mqtt_upload_handle_t *)handle;
    int32_t res = STATE_MQTT_UPLOAD_OPEN_INIT_FAILED;
    uint64_t crc64 = 0;
    uint64_t a = 1;
    ...
    if (read_data_handler == NULL) {
        return STATE_MQTT_UPLOAD_READ_DATA_HANDLER_IS_NULL;
    }

    char temp[MQTT_UPLOAD_DEFAULT_CRC64_LEN] = {0};
    memset(temp, 0, MQTT_UPLOAD_DEFAULT_CRC64_LEN);
    if (digest != NULL) {
        ...
        {
            crc64_temp[7 - i] = ((crc64 / a) % 256) & 0xFF;
            a *= 256;
        }
        core_hex2str(crc64_temp, 8, temp, 1);
    } 
    ...
    core_list_for_each_entry(up_task, &up_handle->file_list, linked_node, upload_file_task_handle_t) {
        if (memcmp(file_name, up_task->file_name, strlen(up_task->file_name)) == 0) {
            /* 请求文件信息已经存在，重新请求 */
            if ((res = _mqtt_upload_send_request_init(up_handle, up_task, file_name, file_size, mode, temp)) == STATE_SUCCESS) {
                up_task->file_size = file_size;
                memcpy(up_task->mode, mode, strlen(mode));
                if (up_task->crc64 != NULL) {
                    memcpy(up_task->crc64, temp, MQTT_UPLOAD_DEFAULT_CRC64_LEN);
                }
                up_task->userdata = userdata;
                up_task->status = STATE_MQTT_UPLOAD_REQUEST_INIT;
                up_task->read_data_handler = read_data_handler;
    ...
    if (up_task != NULL && (res = _mqtt_upload_send_request_init(up_handle, up_task, file_name, file_size, mode, temp)) == STATE_SUCCESS) {
        _mqtt_upload_file_task_init(up_handle->sysdep, up_task, file_name, file_size, mode, temp, read_data_handler, userdata);
        _mqtt_upload_filelist_insert(up_handle, up_task);
    }
    up_handle->sysdep->core_sysdep_mutex_unlock(up_handle->uptask_mutex);
    
    _mqtt_upload_up_task_free(up_handle->sysdep, up_task);
    return res;
}
...
aiot_mqtt_upload_result_t aiot_mqtt_upload_process(void *handle) {
    ...
    uint64_t now = 0;
    aiot_mqtt_upload_result_t result = {
        .code = STATE_MQTT_UPLOAD_NONE
    };
    memset(result.file_name, 0, MQTT_UPLOAD_DEFAULT_FILENAME_LEN);
    if (up_handle == NULL) {
        result.code = STATE_MQTT_UPLOAD_MQTT_HANDLE_IS_NULL;
        return result;
    }
    up_handle->sysdep->core_sysdep_mutex_lock(up_handle->uptask_mutex);
    upload_file_task_handle_t *up_task = NULL, *next = NULL;
    core_list_for_each_entry_safe(up_task, next, &up_handle->file_list, linked_node, upload_file_task_handle_t) {
        ...
        switch (up_task->status)
        {
            case STATE_MQTT_UPLOAD_NONE:
                if (up_task->is_destory == 1) {
                    up_task->is_destory = 0;
                    /* 结束上传，销毁对应的链表内存 */
                    core_log(up_handle->sysdep, STATE_MQTT_UPLOAD_BASE, "Finish upload,Deleted upload task.\r\n");
                    core_list_del(&up_task->linked_node);
                    _mqtt_upload_up_task_free(up_handle->sysdep, up_task);
                }
            ...
            case STATE_MQTT_UPLOAD_REQUEST_INIT:
                /* 请求上传UploadID */
                now = up_handle->sysdep->core_sysdep_time();
            ...
            case STATE_MQTT_UPLOAD_FAILED_MESSAGE_LIGMIT:   /* 消息限流上传失败，可以重新发起请求 */
                /* 回到空闲模式 */
                up_task->status = STATE_MQTT_UPLOAD_NONE;
                up_handle->sysdep->core_sysdep_mutex_unlock(up_handle->uptask_mutex);
                return result;
