...
error_t flash_erase_sector(uint8_t *sector)
{
    ...
    EraseInitStruct.Sector       = sector[0];                          // 要擦除哪个扇区
    ...
    EraseInitStruct.NbSectors    = sector[1];                          // 一次只擦除一个扇区
...
error_t flash_program_word(uint32_t StartAddress, uint32_t data)
{
    ...
    error_t err = OK;
    HAL_FLASH_Unlock(); // 解锁
    if (HAL_OK != HAL_FLASH_Program(FLASH_TYPEPROGRAM_WORD, StartAddress, data)) {
...
error_t flash_program(uint32_t addr, uint8_t *buffer, uint32_t length)
{
    ...
    if (FlashStatus == HAL_OK) {
        ...
        {
            if (HAL_FLASH_Program(FLASH_TYPEPROGRAM_WORD, addr, ((uint32_t *)buffer)[0]) != HAL_OK) // 写入数据
            {
