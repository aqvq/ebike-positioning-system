...
static at_rsp_result_t at_ati_handler(char *rsp)
{
    char *line = strstr(rsp, "Quectel");

    if (line)
    {
        if (sscanf(line, "Quectel%*[\r]%*[\n]%[^\r]", QuectelDeviceType))
        {
...
static int32_t core_at_set_device(at_device_t *device)
{
    ...
    for (i = 0; i < at_handle.device->module_init_cmd_size; i++)
    {
        ...
        {
            at_handle.device->module_init_cmd[i].cmd_len = strlen(at_handle.device->module_init_cmd[i].cmd) - 1;
        }
    }
    for (i = 0; i < at_handle.device->ip_init_cmd_size; i++)
    {
        ...
        {
            at_handle.device->ip_init_cmd[i].cmd_len = strlen(at_handle.device->ip_init_cmd[i].cmd) - 1;
        }
    }
    for (i = 0; i < at_handle.device->open_cmd_size; i++)
    {
        ...
        {
            at_handle.device->open_cmd[i].cmd_len = strlen(at_handle.device->open_cmd[i].cmd) - 1;
        }
    }
    for (i = 0; i < at_handle.device->send_cmd_size; i++)
    {
        ...
        {
            at_handle.device->send_cmd[i].cmd_len = strlen(at_handle.device->send_cmd[i].cmd) - 1;
        }
    }
    for (i = 0; i < at_handle.device->close_cmd_size; i++)
    {
        ...
        {
            at_handle.device->close_cmd[i].cmd_len = strlen(at_handle.device->close_cmd[i].cmd) - 1;
        }
    }

    for (i = 0; i < at_handle.device->poweroff_cmd_size; i++)
    {
        if (at_handle.device->poweroff_cmd[i].cmd != NULL)
        {
            at_handle.device->poweroff_cmd[i].cmd_len = strlen(at_handle.device->poweroff_cmd[i].cmd) - 1;
        }
    }

    for (i = 0; i < at_handle.device->gnss_open_cmd_size; i++)
    {
        if (at_handle.device->gnss_open_cmd[i].cmd != NULL)
        {
            at_handle.device->gnss_open_cmd[i].cmd_len = strlen(at_handle.device->gnss_open_cmd[i].cmd) - 1;
        }
    }

    for (i = 0; i < at_handle.device->gnss_location_cmd_size; i++)
    {
        if (at_handle.device->gnss_location_cmd[i].cmd != NULL)
        {
            at_handle.device->gnss_location_cmd[i].cmd_len = strlen(at_handle.device->gnss_location_cmd[i].cmd) - 1;
        }
    }

    for (i = 0; i < at_handle.device->gnss_close_cmd_size; i++)
    {
        if (at_handle.device->gnss_close_cmd[i].cmd != NULL)
        {
            at_handle.device->gnss_close_cmd[i].cmd_len = strlen(at_handle.device->gnss_close_cmd[i].cmd) - 1;
        }
    }
    for (i = 0; i < at_handle.device->ssl_cmd_size; i++)
    {
        ...
        {
            at_handle.device->ssl_cmd[i].cmd_len = strlen(at_handle.device->ssl_cmd[i].cmd) - 1;
        }
    }

    
    for (i = 0; i < at_handle.device->gnss_agps_open_cmd_size; i++)
    {
        if (at_handle.device->gnss_agps_open_cmd[i].cmd != NULL)
        {
            at_handle.device->gnss_agps_open_cmd[i].cmd_len = strlen(at_handle.device->gnss_agps_open_cmd[i].cmd) - 1;
        }
    }
    for (i = 0; i < at_handle.device->gnss_agps_download_cmd_size; i++)
    {
        if (at_handle.device->gnss_agps_download_cmd[i].cmd != NULL)
        {
            at_handle.device->gnss_agps_download_cmd[i].cmd_len = strlen(at_handle.device->gnss_agps_download_cmd[i].cmd) - 1;
        }
    }
    for (i = 0; i < at_handle.device->gnss_nema_enable_cmd_size; i++)
    {
        if (at_handle.device->gnss_nema_enable_cmd[i].cmd != NULL)
        {
            at_handle.device->gnss_nema_enable_cmd[i].cmd_len = strlen(at_handle.device->gnss_nema_enable_cmd[i].cmd) - 1;
        }
    }
    for (i = 0; i < at_handle.device->gnss_nema_disable_cmd_size; i++)
    {
        if (at_handle.device->gnss_nema_disable_cmd[i].cmd != NULL)
        {
            at_handle.device->gnss_nema_disable_cmd[i].cmd_len = strlen(at_handle.device->gnss_nema_disable_cmd[i].cmd) - 1;
        }
    }
    for (i = 0; i < at_handle.device->gnss_nema_query_cmd_size; i++)
    {
        if (at_handle.device->gnss_nema_query_cmd[i].cmd != NULL)
        {
            at_handle.device->gnss_nema_query_cmd[i].cmd_len = strlen(at_handle.device->gnss_nema_query_cmd[i].cmd) - 1;
        }
    }
    for (i = 0; i < at_handle.device->gnss_state_cmd_size; i++)
    {
        if (at_handle.device->gnss_state_cmd[i].cmd != NULL)
        {
            at_handle.device->gnss_state_cmd[i].cmd_len = strlen(at_handle.device->gnss_state_cmd[i].cmd) - 1;
        }
    }
    for (i = 0; i < at_handle.device->context_state_cmd_size; i++)
    {
        if (at_handle.device->context_state_cmd[i].cmd != NULL)
        {
            at_handle.device->context_state_cmd[i].cmd_len = strlen(at_handle.device->context_state_cmd[i].cmd) - 1;
        }
    }
    for (i = 0; i < at_handle.device->tcp_state_cmd_size; i++)
    {
        if (at_handle.device->tcp_state_cmd[i].cmd != NULL)
        {
            at_handle.device->tcp_state_cmd[i].cmd_len = strlen(at_handle.device->tcp_state_cmd[i].cmd) - 1;
        }
    }
    //----------------------- end---- --------------------------------------
...
static at_rsp_result_t creg_rsp_handler(char *rsp)
{
    char *line = NULL;
    line = strstr(rsp, "+CREG");
    ...
    {        
        ...
        if (line)
        {
            uint8_t n = 0;
            uint8_t stat = 0;            
            if (sscanf(line, "+CREG:%d,%d", (void *)&n, (void *)&stat))
            {
                // printf("n=%d\r\n", n);
                // printf("stat=%d\r\n", stat);
                // 当 AT+CREG? 结果中的<stat>等于1或者5，表示模块已经注册上CS业务
                if (stat == 1 || stat == 5)
                {
                    return AT_RSP_SUCCESS;
                }
    ...
    if (line != NULL)
    {
        ...
        if (line)
        {
            uint8_t n = 0;
            uint8_t stat = 0;            
            if (sscanf(line, "+CEREG:%d,%d", (void *)&n, (void *)&stat))
            {
                // printf("n=%d\r\n", n);
                // printf("stat=%d\r\n", stat);
                // 当 AT+CEREG? 结果中的<stat>等于1或者5，表示模块已经注册上CS业务
                if (stat == 1 || stat == 5)
                {
                    return AT_RSP_SUCCESS;
                }
...
static const core_at_cmd_item_t at_cpin_cmd_table[] = {
    {
        /* 检查SIM卡 */
        .cmd = "AT+CPIN?\r\n",
        .cmd_len = 10, // 此处的命令长度必须设置
        .rsp = "OK",
    },
};

#if (0)
static const core_at_cmd_item_t at_creg_cmd_table[] = {
    {
        .cmd = "AT+CREG?\r\n",
        .cmd_len = 10,
        .rsp = "OK",
        .handler = creg_rsp_handler,
        .timeout_ms = 500,
    },
};
#endif

static const core_at_cmd_item_t at_cereg_cmd_table[] = {
    {
        .cmd = "AT+CEREG?\r\n",
        .cmd_len = 11,
        .rsp = "OK",
...
int32_t aiot_at_bootstrap(void)
{
    ...
    //  通过AT+CPIN?命令查询(U)SIM卡状态
    res = core_at_commands_send_sync(at_cpin_cmd_table, 1);
    if (STATE_SUCCESS != res)
    {
        return res;
    }
...
#if (0)
    // 通过AT+CREG?命令查询CS业务的状态；如果在90s内，没有注册上CS业务，建议重启模块
    timeout_start = os_api->time();
    while (os_api->time() - timeout_start <= 90000)
    {
        res = core_at_commands_send_sync(at_creg_cmd_table, 1);
        if (STATE_SUCCESS == res)
        {
            break;
        }
    ...
    // 60s内通过AT+CGREG?/AT+CEREG?命令查询PS业务的状态
    // 当 AT+CGREG?/AT+CEREG? 结果中的<stat>等于 1 或者5，表示模块已在UMTS/LTE网络注册上PS业务
    // 等待60s后，不管是否注册上PS业务，都进入下一个流程    
    timeout_start = os_api->time();
    while (os_api->time() - timeout_start <= 60000)
    {
        res = core_at_commands_send_sync(at_cereg_cmd_table, 1);
        if (STATE_SUCCESS == res)
        {
            break;
        }
    ...
    for (i = 0; i < at_handle.device->ssl_cmd_size; i++)
    {
        if (at_handle.device->ssl_cmd[i].fmt != NULL)
        {
            if (strstr(at_handle.device->ssl_cmd[i].fmt, "%d"))
            {
                ...
                at_handle.device->ssl_cmd[i].cmd_len = strlen(tmp) - 1;
            }
        }
...
int32_t aiot_at_nwk_connect(uint8_t socket_id, const char *host, uint16_t port, uint32_t timeout)
{
    ...
    at_handle.device->open_cmd[0].cmd_len = strlen(conn_cmd_open) - 1;
...
static int32_t core_at_send_package(uint8_t socket_id, const uint8_t *buffer, uint32_t len, uint32_t timeout)
{
    ...
    at_handle.device->send_cmd[0].cmd_len = strlen(at_handle.device->send_cmd[0].cmd) - 1;
...
int32_t aiot_at_nwk_recv(uint8_t socket_id, uint8_t *buffer, uint32_t len, uint32_t timeout_ms)
{
    ...
    do
    {
        ...
        times++;
        // printf("times: %d, res: %d, len:%d, os_api->time()-timeout_start:%ld\n", times, res, len, (os_api->time()-timeout_start));
        if (res == len)
        {
            break;
        }
...
int32_t aiot_at_poweroff()
{
    ...
    res = core_at_commands_send_sync(at_handle.device->poweroff_cmd, at_handle.device->poweroff_cmd_size);
...
int32_t aiot_at_gnss_open()
{
    ...
    res = core_at_commands_send_sync(at_handle.device->gnss_open_cmd, at_handle.device->gnss_open_cmd_size);
...
int32_t aiot_at_gnss_state()
{
    ...
    res = core_at_commands_send_sync(at_handle.device->gnss_state_cmd, at_handle.device->gnss_state_cmd_size);
...
int32_t aiot_at_context_state()
{
    ...
    res = core_at_commands_send_sync(at_handle.device->context_state_cmd, at_handle.device->context_state_cmd_size);
...
int32_t aiot_at_tcp_state()
{
    ...
    res = core_at_commands_send_sync(at_handle.device->tcp_state_cmd, at_handle.device->tcp_state_cmd_size);
...
int32_t aiot_at_gnss_location()
{
    ...
    res = core_at_commands_send_sync(at_handle.device->gnss_location_cmd, at_handle.device->gnss_location_cmd_size);
...
int32_t aiot_at_gnss_close()
{
    ...
    res = core_at_commands_send_sync(at_handle.device->gnss_close_cmd, at_handle.device->gnss_close_cmd_size);
 ...
 * @return int32_t
 */
int32_t aiot_at_gnss_agps_open()
{
    int32_t res = STATE_SUCCESS;
    if (at_handle.is_init != 1)
    {
    ...
    res = core_at_commands_send_sync(at_handle.device->gnss_agps_open_cmd, at_handle.device->gnss_agps_open_cmd_size);
 ...
 * @return int32_t
 */
int32_t aiot_at_gnss_agps_download()
{
    int32_t res = STATE_SUCCESS;
    if (at_handle.is_init != 1)
    {
    ...
    res = core_at_commands_send_sync(at_handle.device->gnss_agps_download_cmd, at_handle.device->gnss_agps_download_cmd_size);
 ...
 * @return int32_t
 */
int32_t aiot_at_gnss_nema_enable()
{
    int32_t res = STATE_SUCCESS;
    if (at_handle.is_init != 1)
    {
    ...
    res = core_at_commands_send_sync(at_handle.device->gnss_nema_enable_cmd, at_handle.device->gnss_nema_enable_cmd_size);
 ...
 * @return int32_t
 */
int32_t aiot_at_gnss_nema_disable()
{
    int32_t res = STATE_SUCCESS;
    if (at_handle.is_init != 1)
    {
    ...
    res = core_at_commands_send_sync(at_handle.device->gnss_nema_disable_cmd, at_handle.device->gnss_nema_disable_cmd_size);
 ...
 * @return int32_t
 */
int32_t aiot_at_gnss_nema_query()
{
    int32_t res = STATE_SUCCESS;
    if (at_handle.is_init != 1)
    {
    ...
    res = core_at_commands_send_sync(at_handle.device->gnss_nema_query_cmd, at_handle.device->gnss_nema_query_cmd_size);
...
/// @param
/// @return
int32_t aiot_at_tcp_close(void)
{
    int32_t res = STATE_SUCCESS;
    if (at_handle.is_init != 1)
    {
