...
void _usart_recv_isr(UART_HandleTypeDef *huart)
{
    ...
    if (huart == &huart6) {
        ...
        uart_dma_rx_buf.tail = RING_BUFFER_SIZE - hdma_usart6_rx.Instance->NDTR; // cndtr???????
...
{
    // if (HAL_OK == HAL_UART_Transmit_DMA(&huart6, (uint8_t *)p_data, len))
    if (HAL_OK == K_UART_Transmit(&huart6, (uint8_t *)p_data, len)) {
...
static int8_t aiot_at_uart_init(void)
{
    ...
    if (HAL_OK != HAL_UART_Receive_DMA(&huart6, uart_dma_rx_buf.data, RING_BUFFER_SIZE)) {
        Error_Handler();
    }
