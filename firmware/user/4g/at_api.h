 ...
 * @brief AT组件配置参数
 */
#define AIOT_AT_CMD_LEN_MAXIMUM        (256)       /* AT命令最大长度 */
#define AIOT_AT_RSP_LEN_MAXIMUM        (256)       /* AT应答最大长度 */
#define AIOT_AT_TX_TIMEOUT_DEFAULT     (10000)     /* UART默认发送超时时间 */
#define AIOT_AT_RX_TIMEOUT_DEFAULT     (10000)     /* UART默认接受等待超时时间 */
#define AIOT_AT_DATA_RB_SIZE_DEFAULT   (10 * 1024) /* 内部网络数据接收缓冲区大小,用户可根据接收数据流量的大小调整 */
#define AIOT_AT_RSP_RB_SIZE_DEFAULT    (10 * 1024) /* 内部应答报文接受缓冲区大小,用户可根据接收数据流量的大小调整 */
...
typedef struct
{
    ...
    /* 4G模块关机命令列表 */
    core_at_cmd_item_t *poweroff_cmd;
    uint32_t poweroff_cmd_size;

    /* 打开GNSS命令列表, 可选*/
    core_at_cmd_item_t *gnss_open_cmd;
    uint32_t gnss_open_cmd_size;

    /* GNSS定位命令列表, 可选*/
    core_at_cmd_item_t *gnss_location_cmd;
    uint32_t gnss_location_cmd_size;

    /* 关闭GNSS命令列表, 可选*/
    core_at_cmd_item_t *gnss_close_cmd;
    uint32_t gnss_close_cmd_size;

    
    /* 打开APGS功能命令 */
    core_at_cmd_item_t *gnss_agps_open_cmd;
    uint32_t gnss_agps_open_cmd_size;

    /* APGS下载星历数据命令 */
    core_at_cmd_item_t *gnss_agps_download_cmd;
    uint32_t gnss_agps_download_cmd_size;

    /* 使能通过 AT+QGPSGNME 获取 NMEA 语句命令列表, 可选*/
    core_at_cmd_item_t *gnss_nema_enable_cmd;
    uint32_t gnss_nema_enable_cmd_size;

    /* 禁用通过 AT+QGPSGNME 获取 NMEA 语句命令列表, 可选*/
    core_at_cmd_item_t *gnss_nema_disable_cmd;
    uint32_t gnss_nema_disable_cmd_size;

    /* 获取 GGA 语句命令列表, 可选*/
    core_at_cmd_item_t *gnss_nema_query_cmd;
    uint32_t gnss_nema_query_cmd_size;

    /* GNSS状态查询, 可选*/
    core_at_cmd_item_t *gnss_state_cmd;
    uint32_t gnss_state_cmd_size;

    /* 场景状态查询, 可选*/
    core_at_cmd_item_t *context_state_cmd;
    uint32_t context_state_cmd_size;

    /* TCP状态查询, 可选*/
    core_at_cmd_item_t *tcp_state_cmd;
    uint32_t tcp_state_cmd_size;
    //----------------------- end---- --------------------------------------
} at_device_t;
